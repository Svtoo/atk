...
def install_plugin(atk_home: Path, identifier: str) -> int:
    ...
    Returns:
        Exit code from the install command, or 0 if install not defined.
...
class InstallAllResult:
    ...
    def all_succeeded(self) -> bool:
        """Return True if all plugins installed successfully."""
...
def install_all_plugins(atk_home: Path) -> InstallAllResult:
    ...
    Returns:
        InstallAllResult with lists of succeeded and failed plugins.
    """
    ...
    return InstallAllResult(succeeded=succeeded, failed=failed)
